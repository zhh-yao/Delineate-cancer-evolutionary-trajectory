...
def plotImportance(feature, importance):
    ...
    ax1.bar(sorted_feature, sorted_importance, color='cornflowerblue', label='Subclone importance')
    
    ax1.set_yticks(np.arange(0, 0.21, 0.05))
    ax1.set_ylim([0, 0.21])
    ax1.tick_params(bottom=False, length=10, labelsize=25, pad=20)
    plt.xticks(rotation=70)
    plt.legend(loc=(0.4, 0.6), prop=font1, frameon=False)
    ...
    ax2.set_ylim([0, 1.05])
    ax2.tick_params(bottom=False, length=10, labelsize=25, pad=20)
    ax2.set_yticklabels(['0%', '20%', '40%', '60%', '80%', '100%'])
    
    ax1.set_xlabel('Subclones(D=Death, R=Resting, P=Proliferation)', font1, labelpad=20)
    plt.legend(loc=(0.4, 0.5), prop=font1, frameon=False)
