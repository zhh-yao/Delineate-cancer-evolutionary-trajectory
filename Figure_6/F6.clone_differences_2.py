...
def cloneNum(clone, data):
    ...
    dataE = data[clone][:200]
    dataL = data[clone][200:]
    
    return dataE, dataL


def dataCombine(dataE, dataL):
    
    mat = pd.DataFrame(columns=['Pattern', 'Number'])
    
    mat['Pattern'] = ['Aggressive']*len(dataE) + ['Bounded']*len(dataL)
    mat['Number'] = list(dataE) + list(dataL)
...
def boxPlot(data, n):
    ...
    ax.set_ylabel('Proportion', font1, labelpad=20)
    ...
    plt.savefig('box_%d.pdf'%n, bbox_inches='tight')
...
clone = 'D:0.0,R:0.9,P:0.1'
dataE, dataL = cloneNum(clone, X)
data_1 = dataCombine(dataE, dataL)

boxPlot(data_1, 1)
p_1 = stats.kruskal(dataE, dataL)[1]


clone = 'D:0.4,R:0.2,P:0.4'
dataE, dataL = cloneNum(clone, X)
data_2 = dataCombine(dataE, dataL)

boxPlot(data_2, 2)
p_2 = stats.kruskal(dataE, dataL)[1]
