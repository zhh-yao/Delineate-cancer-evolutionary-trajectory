...
def plotDynamic(rowData, computeData, allcycle, Patient, rep):
    ...
    ax.stackplot(time, computeData, colors=colors)
    
    plt.scatter(rowData['Time'], rowData['WBC'], color='k', label='Patient %d'%Patient)
    ...
    plt.savefig('evolution_%d_%d.tif' %(Patient,rep), bbox_inches='tight')
    plt.savefig('evolution_%d_%d.eps' %(Patient,rep), bbox_inches='tight')
    plt.show()
...
def boxPlot(data):
    ...
    plt.text(4.6, 2.5, 'Indeterminate', fontdict=font2)    
    plt.text(13, 2.5, 'Exponential', fontdict=font2)    
    
    ax.tick_params(axis='x',labelsize=20)
    ax.tick_params(axis='y',labelsize=30)
    
    ax.set_xlabel('Patient ID', font1)
