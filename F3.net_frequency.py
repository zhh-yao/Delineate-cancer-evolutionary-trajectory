...
def sub_plot(num_inc, num_dec, Pattern):
    ...
    plt.bar(x, -np.array(num_dec), width=60, color=colors[Pattern], alpha=0.5, hatch='/')
    
    plt.axhline(y=0, lw=1, color='k')
    
    mean_inc = np.mean(num_inc[0:20])
    plt.axhline(y=mean_inc, lw=5, color=colors[Pattern], linestyle='--')
    plt.text(1300, mean_inc+0.3,'mean=%.2f'%mean_inc, fontdict=font2)
    mean_dec = -np.mean(num_dec[0:20])
    plt.axhline(y=mean_dec, lw=5, color=colors[Pattern], alpha=0.5, linestyle='--')
    plt.text(200, mean_dec-0.5,'mean=%.2f'%mean_dec, fontdict=font2)
